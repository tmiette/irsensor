...
public class SupervisorTableModel extends AbstractTableModel {
			...
			public void run() {
				...
				for (SensorNode node : list) {
					if (node.getParentId() == -1) {
						...
						lastIdBooted = node.getId();
						break;
					}
				}
				
				setNodeState(lastIdBooted);
			}
		});
	}
	...
	private void setNodeState(int id){
		//System.out.println("want to connected on "+id);
		List<SensorNode> nodesToConnected = new ArrayList<SensorNode>(); 
		for (SensorNode node : supervisor.getSensorNodes()) {
			if(node.getParentId() == id){
				//System.out.println("node must be connect "+node.getId());
				nodesToConnected.add(node);
			}
		}
		if(nodesToConnected.size() == 0) return;
		for (SensorNode node : nodesToConnected) {
			//System.out.println("Set state node "+node.getId());
			supervisor.setState(node.getId(), SensorState.UP);
			setNodeState(node.getId());
		}
	}
}
