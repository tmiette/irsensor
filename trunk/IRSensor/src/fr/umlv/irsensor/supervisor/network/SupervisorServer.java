...
public class SupervisorServer {
  ...
  public SupervisorServer() {
    try {
      ...
      IRSensorLogger.postMessage(Level.FINE,
          "Supervisor Server is listening on " + serverPort);
    } catch (IOException e) {
          ...
          try {
            ...
            if (DecodePacket.getOpCode(data) == OpCode.REPDATA) {
              try {
                ...
                System.err.println("Invalid packet for Supervisor Server "
                    + e.getMessage());
              }
            } else {
              client.write(PacketFactory
                  .createAck(-1, ErrorCode.INVALID_PACKET));
            }
          } catch (IOException e) {
