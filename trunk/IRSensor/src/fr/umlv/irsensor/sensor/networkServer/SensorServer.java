...
public class SensorServer implements PacketRegisterable {
	...
	public void setPacket(ByteBuffer packet, SocketChannel channel) {
		if (OpCode.getOpcode(packet).equals(OpCode.REQHELLO)) {
		...
		else if(OpCode.getOpcode(packet).equals(OpCode.REQDATA)){
			...
			fireReqDataReceived(reqDataPacket.getCatchArea(), reqDataPacket.getClock(), 
					reqDataPacket.getQuality());
		}
		else if (OpCode.getOpcode(packet).equals(OpCode.REPDATA)){
			try {
