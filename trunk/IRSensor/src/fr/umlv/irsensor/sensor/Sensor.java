			...
			public void repDataReceived(byte[] data, int mimeType) {
				...
				if (dataReceived.size() == children.size()) {
					...
					if (dt == null) {
						...
						try {
							myImage = ImageIO
									.read(new ByteArrayInputStream(dt));
						} catch (IOException e) {
					...
					for (Pair<byte[], Integer> pair : dataReceived) {
						...
						try {
							sonImage = ImageIO.read(new ByteArrayInputStream(
					...
					BufferedImage[] array = new BufferedImage[imagesToMerge
							.size()];
					int i = 0;
					...
					try {
						ImageIO
								.write(result, ViewSight.getFileExtension(),
										bos);
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
		...
		if (this.conf.getParentId() == -1) {
			...
			if (data != null) {
				this.sensorClient.sendRepData(this.conf.getParentAddress(),
						this.conf.getParentId(), 0, data.length, data);
			}
