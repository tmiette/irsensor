...
public class RepDataPacket implements SensorPacket {
	 ...
	 * 
	 * @param bb
	 *            the bytebuffer containing the REPDATA packet
	 * @return an object representing this packet
	 * @throws MalformedPacketException
	 *             if the expecting REPDATA packet is malformed
	 */
	public static RepDataPacket getPacket(ByteBuffer bb)
			throws MalformedPacketException {
		if (bb == null)
			throw new IllegalArgumentException("Empty REPDATA packet");
		ByteBuffer packet = bb.duplicate();
		int index = 0;

		if (packet.capacity() < PacketFields.getLength(PacketFields.OPCODE,
				PacketFields.ID, PacketFields.LENGHT, PacketFields.MIMETYPE)) {
		 ...
		 * datalen) { throw new MalformedPacketException( "No datas were found
		 * in REPDATA"); }
		 */
	...
	public String toString() {
		return "Packet : ID = " + id + " MimeType : " + mimetype
				+ " Data len : " + dataLen;
	}
