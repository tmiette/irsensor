package fr.umlv.irsensor.common;

/**
 * This class represents the sensor configuration values.
 * 
 * @author Miette Tom (tmiette@etudiant.univ-mlv.fr)
 * @author Moreau Alan (amorea04@etudiant.univ-mlv.fr)
 * @author Mouret Sebastien (smouret@etudiant.univ-mlv.fr)
 * @author Pons Julien (jpons@etudiant.univ-mlv.fr)
 */
public interface IRSensorConfiguration {

	public static final int SUPERVISOR_SERVER_PORT = 31000;

	public static final int SENSOR_SERVER_PORT = 31001;

	public static final int DATA_SERVER_PORT = 31002;

	public static final int SERVER_PORT_LOCAL = 32000;

	public static final int PACKET_MAX_SIZE = 32768;
}
